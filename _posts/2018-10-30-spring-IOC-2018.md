...
# AOP
    动态代理，指在冲虚运行期间动态的将某段代码切入到指定方法指定位置进行运行的编程方式。
    ...
    7、在配置类中加@EnableAspectJAutoProxy开启基于注解的AOP模式
...
AOP原理：
    ...
    1.@EnableAspectJAutoProxy是什么？
        @Import(AspectJAutoProxyRegistrar.class)给容器中导入AutoProxyRegistrar
    ...
    关注后置处理器(在bean初始化前后完成的事情)、自动装配BeanFactory
---
### ---@Pointcut
标注切点
例：
